...
<strong><?=$number_of_ads?> Ad Banner(s)</strong>.  &nbsp; &nbsp; &nbsp; Showing <?=$first_show?> - <?=$last_show?>
<br>
<br />
...
<?
	if ($number_of_ads > 0) 	// any ads ?
	{
		...
		while ($rs = mysql_fetch_object($result))
		{
			$ads_id = $rs->ads_id;
			$title = $rs->title;
			$ad_image = stripslashes($rs->ad_image);
?>
  			<tr onmouseover="this.className='editfocus';" onmouseout="this.className='editblur';">
                <td align="left" >&nbsp;<?=$title?></td>
                <td align="left" ><img src="/ad_banners/<?=$ad_image?>" height="50" /></td>
    
                <td align="center">
                <a class="textlink" href="edit-banner-ad.php?id=<?=$ads_id?>&tk=<?=$_SESSION["token"];?>"><img src="/images/members/button-edit.jpg" width="40" height="15" alt="Edit" border="0"  /></a>
                <a class="textlink" href="delete-banner-ad.php?id=<?=$ads_id?>&tk=<?=$_SESSION["token"];?>" onclick="return verify_delete('<?=$title?>');"><img src="/images/members/button-delete.jpg" width="40" height="15" alt="Delete"  border="0" /></a>
              </td>
            </tr>
<?
            $row_counter++;
