...
if ($token == $_SESSION['token'] && $sid == session_id())
{
    if ($job_title || $job_title_id || $search_terms)
    {
        ...
        if ($job_title) {
            ...
            if ($debug_msgs) {
                echo "<strong>JOB TITLE</strong>: $query2<br />";
            }
        ...
        if ($job_title_id) {
            ...
            if ($debug_msgs) {
                echo "<strong>JOB TITLE ID</strong>: $query2<br />";
            }
            $result2 = mysql_query($query2) or die(mysql_error());
            $num_found = mysql_num_rows($result2);
        ...
        if (trim($search_terms)) {
            ...
            if ($debug_msgs) {
                echo "<strong>KEYWORDS</strong>: $query2<br />";
            }
            $result2 = mysql_query($query2) or die(mysql_error());
            $num_found = mysql_num_rows($result2);
        ...
        switch ($_GET["sort"]) {
            ...
            case "salary":
                $salary_min_or_max = ($_GET["rev"]) ? "salary_max" : "salary_min";
                ...
                break;
            case "expires":
                $sort_order = " jobs.date_expires $desc_for_rev";
                break;
            case "title":
            default:
                $sort_order = " jobs.job_title $desc_for_rev";
                break;
        }
        ...
        if ($debug_msgs) {
            echo "number_of_jobs = $number_of_jobs<br />";
        }
        ...
        ?>
            ...
            <p>We're sorry, but we can't seem to find any jobs that match your search criteria.
            </p>
        <?
        } else {
