...
if ($_GET["restore_search"] == 1)
{
	 ...
	 $hourly_rate_min 	= $_SESSION["hourly_rate_min"];
	 $hourly_rate_max 	= $_SESSION["hourly_rate_max"];
...
else if ($_GET["new_search"] == 1)
{
	...
	$_SESSION["hourly_rate_min"] = $hourly_rate_min = '';
	$_SESSION["hourly_rate_max"] = $hourly_rate_max = '';
...
else // Search has been submitted from search form, so save parms.
{
	...
	$_SESSION["hourly_rate_min"] = $hourly_rate_min = clean_post_var($_GET["hourly_rate_min"]);
	$_SESSION["hourly_rate_max"] = $hourly_rate_max = clean_post_var($_GET["hourly_rate_max"]);
...
if ($debug_msgs)
{
	...
	echo "hourly_rate_min = $hourly_ratehourly_rate_minbr />";	
...
function change_search_type(which)
{
    ...
    <form action="search-candidates.php" method="get"  name="search">
...
<span id="form_category" style="display:<? if ($search_type == 'a') echo 'block'; else echo "none"; ?>"></span>
<? /*
job_title
profile_bio
education
...
qualifications
  ...
  <tr>
    <td  height="29"></td>
    <td align="left"><sup>*</sup><em style="color:#999">Select and/or enter job title above.</em></td>
  </tr>
    </table>
*/ ?>
<span id="form_sallary" style="display:<? if ($search_type == 'a') echo 'block'; else echo "none"; ?>">
  ...
  <tr>
    ...
    <td align="left">$<input name="hourly_rate_min" id="hourly_rate_min" type="text" value="<?=$hourly_rate_min?>" style="width:50px">
    				to
                    $<input name="hourly_rate_max" id="hourly_rate_max" type="text" value="<?=$hourly_rate_max?>" style="width:50px"></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	?>
    <td align="left"><select name="years_min" id="years_min" style="width:60px;" >
      <option selected="selected" value="0">Any</option>
        <?
        ...
        to:
        <select name="years_max" id="years_max"  style="width:60px;">
          <option selected="selected" value="0">Any</option>
            <?
              for ($years = 1; $years <= $max_for_years_range; $years++)
              {
